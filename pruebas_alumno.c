...
void prueba_abb_integral()
{
	...
	for(int i = 0; i < 5; i++){
		fue_quitado = false;
		fue_quitado = abb_quitar(arbol, &quitar_cinco_elem[i], &elem_quitado);
		pa2m_afirmar(fue_quitado, "Fue quitado %d", *(int*)elem_quitado);
	...
	for(int i = 0; i < 5; i++)
		pa2m_afirmar(cinco_inorden_esperados[i] == *(int*)cinco_obtenidos[i],
			   "Esperado %d <-> Obtenido %d",
			   cinco_inorden_esperados[i], *(int*)cinco_obtenidos[i]);
	printf(NORMAL "PREORDEN: \n");
	abb_vectorizar_preorden(arbol, cinco_obtenidos, 5);
	for(int i = 0; i < 5; i++)
		pa2m_afirmar(cinco_preorden_esperados[i] == *(int*)cinco_obtenidos[i],
			   "Esperado %d <-> Obtenido %d",
			   cinco_preorden_esperados[i], *(int*)cinco_obtenidos[i]);
	printf(NORMAL "POSTORDEN: \n");
	abb_vectorizar_postorden(arbol, cinco_obtenidos, 5);
	for(int i = 0; i < 5; i++)
		pa2m_afirmar(cinco_postorden_esperados[i] == *(int*)cinco_obtenidos[i],
			   "Esperado %d <-> Obtenido %d",
			   cinco_postorden_esperados[i], *(int*)cinco_obtenidos[i]);
	...
	for(int i = 0; i < 8; i++)
		pa2m_afirmar(ocho_inorden_esperados[i] == *(int*)ocho_obtenidos[i],
			   "Esperado %d <-> Obtenido %d",
			   ocho_inorden_esperados[i], *(int*)ocho_obtenidos[i]);
	printf(NORMAL "PREORDEN: \n");
	abb_vectorizar_preorden(arbol, ocho_obtenidos, 5);
	for(int i = 0; i < 8; i++)
		pa2m_afirmar(ocho_preorden_esperados[i] == *(int*)ocho_obtenidos[i],
			   "Esperado %d <-> Obtenido %d",
			   ocho_preorden_esperados[i], *(int*)ocho_obtenidos[i]);
	printf(NORMAL "POSTORDEN: \n");
	abb_vectorizar_postorden(arbol, ocho_obtenidos, 5);
	for(int i = 0; i < 8; i++)
		pa2m_afirmar(ocho_postorden_esperados[i] == *(int*)ocho_obtenidos[i],
			   "Esperado %d <-> Obtenido %d",
			   ocho_postorden_esperados[i], *(int*)ocho_obtenidos[i]);
